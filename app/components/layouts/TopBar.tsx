...
const TopBar = ({home,about,works,contact, scrollToComponent = ()=>{}}:any) => {
    ...
    const isActive = "font-medium text-theme-orange hover:text-theme-orange";
    const notActive = "text-theme-blue hover:text-theme-orange";

  return (
    <div className={`flex w-full justify-between items-center py-1 lg:py-5 px-4 xl:px-14  fixed top-0 left-0 z-20 transition duration-200   ${scrollPosition != 0 ? 'bg-[#fff]': 'bg-transparent'} `}>
        ...
        <span className='hidden md:flex items-center gap-10'>
                <button onClick={()=>{scrollToComponent(home), setTab(1)}} className={tab == 1 ? isActive : notActive}>
                    <p className='text-[15px] '>Home</p>
                </button>
                <button onClick={()=>{scrollToComponent(about),setTab(2)}} className={tab == 2 ? isActive : notActive}>
                    <p className='text-[15px]'>About Me</p>
                </button>
                <button onClick={()=>{scrollToComponent(works), setTab(3)}} className={tab == 3 ? isActive : notActive}>
                    <p className='text-[15px]'>Porfolio</p>
                </button>
                <button onClick={()=>{scrollToComponent(contact), setTab(4)}} className={tab == 4 ? isActive : notActive}>
        ...
        <span className='flex items-center gap-4 justify-center leading-4'>
                <Link href={'https://www.facebook.com/marcelito.cos'} target='_blank' >
                    <button className='text-theme-blue hover:text-theme-orange'>
                        <Icon icon='fa-brands:facebook' width={23} />
                    </button>
                </Link>
                <Link href={'https://www.linkedin.com/in/marcelito-cosicol-19288b22b/'} target='_blank' >
                    <button className='text-theme-blue hover:text-theme-orange'>
